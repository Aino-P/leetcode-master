...
public class Main{
    static int[][] grid;
    static int ex, ey;  //终点
    static int[][] dirs = {{-1, -2},{-2, -1},{-1, 2},{-2, 1}, {2, -1},{1, -2}, {2, 1}, {1, 2}};
    ...
    public static int Heuristic(Knight k) { // 欧拉距离
        return (k.x - ex) * (k.x - ex) + (k.y - ey) * (k.y - ey); // 统一不开根号，这样可以提高精度
    }
    public static void aStar(int bx, int by){
//        Queue<int[]> q = new LinkedList<>();
        PriorityQueue<Knight> pq = new PriorityQueue<>();
        Knight start = new Knight(bx, by);
        start.g = 0;
        start.h = Heuristic(start);
        start.f = start.g + start.h;
        pq.add(start);
        while(!pq.isEmpty()){
            ...
            if(cur.x == ex && cur.y == ey){
                System.out.println(grid[cur.x][cur.y]);
                return;
            }
            for(int i = 0; i < 8; i++) {
                Knight next = new Knight(cur.x + dirs[i][0], cur.y + dirs[i][1]);
                if(next.x < 1 || next.x >= 1000 || next.y < 1 || next.y >= 1000)
                    continue;
